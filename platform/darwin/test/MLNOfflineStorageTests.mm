...
+ (void)tearDown {
  ...
  XCTAssertTrue([[NSFileManager defaultManager] fileExistsAtPath:cacheDirectoryURL.path],
                @"Directory containing database should exist.");
  ...
  XCTAssertFalse([[NSFileManager defaultManager] fileExistsAtPath:cacheURL.path],
...
- (void)setUp {
  ...
  dispatch_once(&onceToken, ^{
    XCTestExpectation *expectation = [self
        keyValueObservingExpectationForObject:[MLNOfflineStorage sharedOfflineStorage]
                                      keyPath:@"packs"
                                      handler:^BOOL(id _Nonnull observedObject,
                                                    NSDictionary *_Nonnull change) {
                                        const auto changeKind = static_cast<NSKeyValueChange>(
                                            [change[NSKeyValueChangeKindKey] unsignedLongValue]);
    ...
    XCTAssertNotNil(
        ...
        @"Shared offline storage object should have a non-nil collection of packs by this point.");
  });
}
...
- (void)testSharedObject {
  XCTAssertEqual([MLNOfflineStorage sharedOfflineStorage], [MLNOfflineStorage sharedOfflineStorage],
                 @"There should only be one shared offline storage object.");
}
...
- (void)testAddPackForBounds {
  ...
  MLNTilePyramidOfflineRegion *region =
      [[MLNTilePyramidOfflineRegion alloc] initWithStyleURL:styleURL
                                                     bounds:bounds
  ...
  [self keyValueObservingExpectationForObject:[MLNOfflineStorage sharedOfflineStorage]
                                      keyPath:@"packs"
                                      handler:^BOOL(id _Nonnull observedObject,
                                                    NSDictionary *_Nonnull change) {
                                        const auto changeKind = static_cast<NSKeyValueChange>(
                                        ...
                                        return changeKind == NSKeyValueChangeInsertion &&
                                               indices.count == 1;
                                      }];
  XCTestExpectation *additionCompletionHandlerExpectation =
      [self expectationWithDescription:@"add pack completion handler"];
  [[MLNOfflineStorage sharedOfflineStorage]
       addPackForRegion:region
            withContext:context
      completionHandler:^(MLNOfflinePack *_Nullable completionHandlerPack,
                          NSError *_Nullable error) {
        XCTAssertNotNil(completionHandlerPack, @"Added pack should exist.");
        XCTAssertEqual(completionHandlerPack.state, MLNOfflinePackStateInactive,
                       @"New pack should initially have inactive state.");
        pack = completionHandlerPack;
        [additionCompletionHandlerExpectation fulfill];
      }];
  [self waitForExpectationsWithTimeout:5 handler:nil];

  XCTAssertEqual([MLNOfflineStorage sharedOfflineStorage].packs.count, countOfPacks + 1,
                 @"Added pack should have been added to the canonical collection of packs owned by "
                 @"the shared offline storage object. This assertion can fail if this test is run "
                 @"before -testAAALoadPacks.");

  XCTAssertEqual(pack, [MLNOfflineStorage sharedOfflineStorage].packs.lastObject,
                 @"Pack should be appended to end of packs array.");

  XCTAssertEqualObjects(pack.region, region, @"Added pack’s region has changed.");

  NSDictionary *userInfo = [NSKeyedUnarchiver unarchiveObjectWithData:pack.context];
  XCTAssert([userInfo isKindOfClass:[NSDictionary class]],
            @"Context of offline pack isn’t a dictionary.");
  XCTAssert([userInfo[nameKey] isKindOfClass:[NSString class]],
            @"Name of offline pack isn’t a string.");
  XCTAssertEqualObjects(userInfo[nameKey], name, @"Name of offline pack has changed.");

  XCTAssertEqual(pack.state, MLNOfflinePackStateInactive,
                 @"New pack should initially have inactive state.");

  [self keyValueObservingExpectationForObject:pack
                                      keyPath:@"state"
                                      handler:^BOOL(id _Nonnull observedObject,
                                                    NSDictionary *_Nonnull change) {
                                        const auto changeKind = static_cast<NSKeyValueChange>(
                                            [change[NSKeyValueChangeKindKey] unsignedLongValue]);
                                        const auto state = static_cast<MLNOfflinePackState>(
                                            [change[NSKeyValueChangeNewKey] longValue]);
                                        return changeKind == NSKeyValueChangeSetting &&
                                               state == MLNOfflinePackStateInactive;
                                      }];
  [self
      expectationForNotification:MLNOfflinePackProgressChangedNotification
                          object:pack
                         handler:^BOOL(NSNotification *_Nonnull notification) {
                           MLNOfflinePack *notificationPack = notification.object;
                           XCTAssert([notificationPack isKindOfClass:[MLNOfflinePack class]],
                                     @"Object of notification should be an MLNOfflinePack.");

                           NSDictionary *userInfo = notification.userInfo;
                           XCTAssertNotNil(
                               userInfo,
                               @"Progress change notification should have a userInfo dictionary.");

                           NSNumber *stateNumber = userInfo[MLNOfflinePackUserInfoKeyState];
                           XCTAssert(
                               [stateNumber isKindOfClass:[NSNumber class]],
                               @"Progress change notification’s state should be an NSNumber.");
                           XCTAssertEqual(stateNumber.integerValue, pack.state,
                                          @"State in a progress change notification should match "
                                          @"the pack’s state.");

                           NSValue *progressValue = userInfo[MLNOfflinePackUserInfoKeyProgress];
                           XCTAssert(
                               [progressValue isKindOfClass:[NSValue class]],
                               @"Progress change notification’s progress should be an NSValue.");
                           XCTAssertEqualObjects(
                               progressValue,
                               [NSValue valueWithMLNOfflinePackProgress:pack.progress],
                               @"Progress change notification’s progress should match pack’s "
                               @"progress.");

                           return notificationPack == pack &&
  ...
  XCTAssertEqualObjects(
      ...
      @"Offline pack context should match the context specified by the application.");
  NSString *newName = @"🍑 Peach Grove";
  NSData *newContext = [NSKeyedArchiver archivedDataWithRootObject:@{
    nameKey : newName,
  }];

  XCTestExpectation *contextCompletionHandlerExpectation =
      [self expectationWithDescription:@"set pack completion context handler"];
  __weak MLNOfflinePack *weakPack = pack;
  [pack setContext:newContext
      completionHandler:^(NSError *_Nullable error) {
        XCTAssertNil(error);
        XCTAssertNotNil(weakPack);
        XCTAssertEqualObjects(
            weakPack.context, newContext,
            @"Offline pack context should match the updated context specified by the application.");
...
- (void)testAddPackForGeometry {
  ...
  double zoomLevel = 20;
  NSString *geojson =
      @"{ \"type\": \"Polygon\", \"coordinates\": [ [ [ 5.1299285888671875, 52.10365839097971 ], [ "
      @"5.103063583374023, 52.110037078604236 ], [ 5.080232620239258, 52.09548601177304 ], [ "
      @"5.106925964355469, 52.07987524347506 ], [ 5.1299285888671875, 52.10365839097971 ] ] ]}";
  NSError *error;
  MLNShape *shape = [MLNShape shapeWithData:[geojson dataUsingEncoding:NSUTF8StringEncoding]
                                   encoding:NSUTF8StringEncoding
                                      error:&error];
  XCTAssertNil(error);
  MLNShapeOfflineRegion *region = [[MLNShapeOfflineRegion alloc] initWithStyleURL:styleURL
                                                                            shape:shape
                                                                    fromZoomLevel:zoomLevel
                                                                      toZoomLevel:zoomLevel];
...
- (void)testInvalidatePack {
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"Expect offline pack to be invalidated without an error."];
  MLNCoordinateBounds bounds = {
  ...
  [[MLNOfflineStorage sharedOfflineStorage]
      ...
      completionHandler:^(MLNOfflinePack *_Nullable pack, NSError *_Nullable error) {
        ...
        [[MLNOfflineStorage sharedOfflineStorage] invalidatePack:pack
                                           withCompletionHandler:^(NSError *_Nullable) {
                                             XCTAssertNotNil(pack);
                                             XCTAssertNil(error);
                                             [expectation fulfill];
                                           }];
...
- (void)testRemovePack {
  ...
  XCTestExpectation *completionHandlerExpectation =
      [self expectationWithDescription:@"remove pack completion handler"];
  [[MLNOfflineStorage sharedOfflineStorage]
                 removePack:pack
      withCompletionHandler:^(NSError *_Nullable error) {
        XCTAssertEqual(pack.state, MLNOfflinePackStateInvalid,
                       @"Removed pack should be invalid in the completion handler.");
        [completionHandlerExpectation fulfill];
      }];
  [self waitForExpectationsWithTimeout:5 handler:nil];

  XCTAssertEqual(pack.state, MLNOfflinePackStateInvalid,
                 @"Removed pack should have been invalidated synchronously.");

  XCTAssertEqual([MLNOfflineStorage sharedOfflineStorage].packs.count, countOfPacks - 1,
                 @"Removed pack should have been removed from the canonical collection of packs "
                 @"owned by the shared offline storage object. This assertion can fail if this "
                 @"test is run before -testAAALoadPacks or -testAddPack.");
}
...
- (void)testBackupExclusion {
  ...
  cacheDirectoryURL = [cacheDirectoryURL URLByAppendingPathComponent:@".mapbox"];
  XCTAssertTrue([[NSFileManager defaultManager] fileExistsAtPath:cacheDirectoryURL.path],
                @"Cache subdirectory should exist.");

  NSURL *cacheURL = [cacheDirectoryURL URLByAppendingPathComponent:@"cache.db"];
  XCTAssertTrue([[NSFileManager defaultManager] fileExistsAtPath:cacheURL.path],
                @"Cache database should exist.");
  ...
  [cacheDirectoryURL getResourceValue:&exclusionFlag
                               forKey:NSURLIsExcludedFromBackupKey
                                error:&error];
  XCTAssertTrue(
      exclusionFlag && [exclusionFlag boolValue],
      @"Backup exclusion flag should be set for the directory containing the cache database.");
  ...
  for (int i = 0; i < count; i++) {
    ...
    MLNTilePyramidOfflineRegion *region =
        [[MLNTilePyramidOfflineRegion alloc] initWithStyleURL:styleURL
                                                ...
                                                fromZoomLevel:20
                                                  toZoomLevel:20];
    NSData *context = [NSKeyedArchiver archivedDataWithRootObject:@{@"index" : @(i)}];
...
- (void)test15536RemovePacksOnBackgroundQueueWhileReloading {
  ...
  dispatch_queue_t queue =
      dispatch_queue_create("com.mapbox.testRemovePacks", DISPATCH_QUEUE_SERIAL);
...
- (void)testAddFileContent {
  ...
  // Valid database
  [XCTContext
      runActivityNamed:@"Valid database"
                 block:^(id<XCTActivity> activity) {
                   NSURL *resourceURL =
                       [NSURL fileURLWithPath:[[NSBundle bundleForClass:[self class]]
                                                  pathForResource:@"sideload_sat"
                                                           ofType:@"db"]];

                   NSError *error;
                   NSDictionary *fileAttributes =
                       [fileManager attributesOfItemAtPath:resourceURL.path error:&error];
                   XCTAssertNil(error,
                                @"Getting the file's attributes should not return an error. (%@)",
                                resourceURL.path);

                   NSNumber *fileSizeNumber = [fileAttributes objectForKey:NSFileSize];
                   long long fileSize = [fileSizeNumber longLongValue];
                   long long databaseFileSize = 19218432;
                   // Merging databases creates an empty file if the file does not exist at the
                   // given path.
                   XCTAssertEqual(fileSize, databaseFileSize,
                                  @"The database file size must be:%lld actual size:%lld",
                                  databaseFileSize, fileSize);

                   NSUInteger countOfPacks = [MLNOfflineStorage sharedOfflineStorage].packs.count;

                   [self
                       keyValueObservingExpectationForObject:[MLNOfflineStorage
                                                                 sharedOfflineStorage]
                                                     keyPath:@"packs"
                                                     handler:^BOOL(id _Nonnull observedObject,
                                                                   NSDictionary *_Nonnull change) {
                                                       const auto changeKind =
                                                           static_cast<NSKeyValueChange>(
                   ...
                   XCTestExpectation *fileAdditionCompletionHandlerExpectation =
                       [self expectationWithDescription:@"add database content completion handler"];
                   MLNOfflineStorage *os = [MLNOfflineStorage sharedOfflineStorage];
                   [os addContentsOfURL:resourceURL
                       withCompletionHandler:^(NSURL *fileURL,
                                               NSArray<MLNOfflinePack *> *_Nullable packs,
                                               NSError *_Nullable error) {
                         XCTAssertNotNil(fileURL, @"The fileURL should not be nil.");
                         XCTAssertNotNil(
                             packs,
                             @"Adding the contents of the sideload_sat.db should update one pack.");
                         XCTAssertNil(error,
                                      @"Adding contents to a file should not return an error.");
                         for (MLNOfflinePack *pack in [MLNOfflineStorage sharedOfflineStorage]
                                  .packs) {
                           NSLog(@"PACK:%@", pack);
                         }
                   ...
                   [self waitForExpectationsWithTimeout:10 handler:nil];
                   // Depending on the database it may update or add a pack. For this case
                   // specifically the offline database adds one pack.
  ...
  [XCTContext
      runActivityNamed:@"Invalid database type"
                 block:^(id<XCTActivity> activity) {
                   NSURL *resourceURL =
                       [NSURL fileURLWithPath:[[NSBundle bundleForClass:[self class]]
                                                  pathForResource:@"one-liner"
                                                           ofType:@"json"]];

                   XCTestExpectation *invalidFileCompletionHandlerExpectation = [self
                       expectationWithDescription:@"invalid content database completion handler"];
                   MLNOfflineStorage *os = [MLNOfflineStorage sharedOfflineStorage];
                   [os addContentsOfFile:resourceURL.path
                       withCompletionHandler:^(NSURL *fileURL,
                                               NSArray<MLNOfflinePack *> *_Nullable packs,
                                               NSError *_Nullable error) {
                         XCTAssertNotNil(
                             error,
                             @"Passing an invalid offline database file should return an error.");
                         XCTAssertNil(packs, @"Passing an invalid offline database file should not "
                                             @"add packs to the offline database.");
  ...
  // File does not exist
  [XCTContext runActivityNamed:@"File does not exist"
                         block:^(id<XCTActivity> activity) {
                           NSURL *resourceURL = [NSURL URLWithString:@"nonexistent.db"];

                           MLNOfflineStorage *os = [MLNOfflineStorage sharedOfflineStorage];
                           XCTAssertThrowsSpecificNamed(
                               [os addContentsOfURL:resourceURL withCompletionHandler:nil],
                               NSException, NSInvalidArgumentException,
                               "MLNOfflineStorage should rise an exception if an invalid database "
                               "file is passed.");
                         }];

  // URL to a non-file
  [XCTContext runActivityNamed:@"URL to a non-file"
                         block:^(id<XCTActivity> activity) {
                           ...
                           XCTAssertThrowsSpecificNamed(
                               [os addContentsOfURL:resourceURL withCompletionHandler:nil],
                               NSException, NSInvalidArgumentException,
                               "MLNOfflineStorage should rise an exception if an invalid URL file "
                               "is passed.");
                         }];
}
  ...
  [os preloadData:data
       ...
       modificationDate:nil
         expirationDate:nil
                   eTag:nil
         mustRevalidate:NO
      completionHandler:^(NSURL *url, NSError *_Nullable error) {
  ...
  [os preloadData:data
      ...
      completionHandler:^(NSURL *url, NSError *_Nullable error) {
        ...
        req = fs->request(resource, [&, tNow = now.timeIntervalSince1970,
          ...
          XCTAssertEqual(MLNTimeIntervalFromDuration(res.modified->time_since_epoch()), floor(tNow),
                         @"Modification timestamp should roundtrip");
          XCTAssertTrue(res.expires, @"Request should have an expiration timestamp");
          XCTAssertEqual(MLNTimeIntervalFromDuration(res.expires->time_since_epoch()),
                         floor(tFuture), @"Expiration timestamp should roundtrip");
...
- (void)testSetMaximumAmbientCache {
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"Expect maximum cache size to be raised without an error."];
  [[MLNOfflineStorage sharedOfflineStorage]
      setMaximumAmbientCacheSize:0
           withCompletionHandler:^(NSError *_Nullable error) {
             XCTAssertNil(error);
             [[MLNOfflineStorage sharedOfflineStorage]
                 setMaximumAmbientCacheSize:50 * 1024 * 1024
                      withCompletionHandler:^(NSError *_Nullable error) {
                        XCTAssertNil(error);
                        [expectation fulfill];
                      }];
...
- (void)testInvalidateAmbientCache {
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"Expect cache to be invalidated without an error."];
  [[MLNOfflineStorage sharedOfflineStorage]
      invalidateAmbientCacheWithCompletionHandler:^(NSError *_Nullable error) {
        XCTAssertNil(error);
        [expectation fulfill];
      }];
  [self waitForExpectationsWithTimeout:10 handler:nil];
...
- (void)testClearCache {
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"Expect cache to be cleared without an error."];
  [[MLNOfflineStorage sharedOfflineStorage]
      clearAmbientCacheWithCompletionHandler:^(NSError *_Nullable error) {
        XCTAssertNil(error);
        [expectation fulfill];
      }];
  [self waitForExpectationsWithTimeout:10 handler:nil];
...
- (void)testResetDatabase {
  XCTestExpectation *expectation =
      [self expectationWithDescription:@"Expect database to be reset without an error."];
  [[MLNOfflineStorage sharedOfflineStorage]
